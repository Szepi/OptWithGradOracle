...
In the algorithm, we will assume that the regularizer function $\mathcal{R}$ is $\alpha$-strongly convex, the target function $f$ is $L$-smooth and $\mu$-strongly convex.
...
\For{$t=1, 2, \cdots, n-1$}	
	...
	$$
\EndFor
\State {\bf Return:} $\hat{X}_n = \dfrac{1}{n}\sum_{t=1}^n X_t \,.$
\end{algorithmic}
...
If the class of functions is also $\mu$-strongly convex w.r.t. $\mathcal{R}$, and $\mu > \dfrac{2L}{\alpha}$. Choose
\[
 \eta_t = \dfrac{2}{\mu t} \,,
\]
%\[
