...
\multirow{2}{*}{$\bm{f \in \C^3}$} & \multirow{2}{*}{$(C_1 \delta^2, \frac{C_2}{\delta^2})$} & \multirow{2}{*}{$(C_1 \delta^2, \frac{C_2}{\delta^2})$} \\
 &&\\\bottomrule
\end{tabular}
\caption{Gradient oracles for different function classes and noise categories. Each table entry specifies the pair $(c_1(\delta), c_2(\delta))$, with $C_1, C'_1,C_2$ being problem dependent constants.\protect\footnotemark
\label{tab:oracles}
}
\end{table}
\footnotetext{
...
Then $\gamma$ is a type-I oracle with $c_1(\delta)$ and $c_2(\delta)$ given in \cref{tab:oracles}.
\end{proposition}
%\begin{proof}
%See Section \ref{sec:appendix-grad}.
%\end{proof}
