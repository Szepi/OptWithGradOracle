...
\label{eq:controlled}
Z_t = F(X_t,U_t),
\end{equation} where $U_t\in \R$ is chosen by the algorithm and $\EE{F(x,U)}=f(X)$ for some distribution $P_U$ over the reals. That is the algorithm can call the oracle $F(x,u)$ by selecting both $x$ and $u$. Note that the algorithm is aware of $P_U$, but does not know how different values of $u$ affect the noise $\xi(x,u)=F(x,u)-f(x)$. Nevertheless, the algorithm can control the noise by selecting $U_t$, and hence it can obtain unbiased estimates of the function values by selecting $U_t$ from $P_U$; we will refer to this case as the \emph{controlled noise} setting and to other situations as the case of \emph{uncontrolled noise}. The advantage of the controlled noise setting is that the algorithm may obtain samples of $F$ at $X^+$ and $X^-$, with the same noise levels:
