...
In the proof, we use \cref{lem:ub} to deal with noisy and biased gradient, which is essentially Theorem~C.4 of \cite{MahdaviPhd:2014}, and also identical to Theorem~6.3 of \cite{Bu:Convex14}, who cites \citet{Dekel:minibatch12} as the source.
...
\begin{lemma}
\label{lem:ub}
Let $({\cF}_t)_{t}$ be a filtration such that $X_t$ is ${{\cF}}_t$-measurable.
...
sequence $(a_t)_{t=1}^{n-1}$ of numbers. Then, if ${f}$ is convex and $L$-smooth,
\begin{align*}
...
If ${{f}}$ is also $\mu$-strongly convex w.r.t.\  $\mathcal{R}$, which is $\alpha$-strongly convex, $\alpha > \dfrac{2L}{\mu}$. Let $\eta_t = \dfrac{2}{\mu t}$, $a_t = \dfrac{\alpha \mu}{2}t-L > 0$,
\begin{align*}
 \EE{ \sum_{t=1}^n {f}(X_t) - {f}(x) }
\le 	 \EE{{f}(X_1)-{f}(x)}+
 \sqrt{\tfrac{2D}{\alpha}} \sum_{t=1}^{n-1} \beta_t
...
Writing
$\nabla f = (\nabla f-\overline G_t)  + \xi_t + G_t$ where $\xi_t = \overline G_t - G_t$ is the ``noise'',
from the convexity of $f$ we obtain
\begin{align}
f(X_{t}) - f(x) 
 &\le \ip{\nabla f(X_t),X_{t}-x} \nonumber  \\
&= \ip{(\nabla f-\overline G_t)  + \xi_t + G_t,X_t-x} \nonumber \\
%& \le \norm{X_t-x}\norm{\nabla f-\overline G_t}_* + \ip{\xi_t,X_{t}-x} + \ip{ G_t, X_{t} - x }
%&\le \beta_t \norm{X_{t+1}-x} + \\
...
\subsection{Online optimization based on \citet{saha2011improved}}
Assume that $\tf$ in the $(c_1,c_2)$ type-II oracle is convex with gradients bounded by $M$. 
...
\]
Using the linearization $\tf(X_t) - \tf(x) \le \ip{\Delta \tf_t,X_t-x} = \EE{\ip{ G_t,X_t-x}| \cF_t}$, summing up the bounds for all $t$, and applying \eqref{eq:div-telescope}, we get the standard mirror descent bounnd:
\begin{align}
...
& \le \EE{\sum_{t=1}^n \tf_t(X_t)} - \inf_{x \in \cK} \sum_{t=1}^n \tf_t(x) + 2 n c_1(\delta) + \delta n M \\
& \le \frac{D}{\eta_{n-1}}  + \sum_{t=1}^n \eta_t \frac{2M^2 + 2 c_2(\delta)^2}{2\alpha} +  2 n c_1(\delta) + \delta n M
\end{align*}
Here we bound $\tf(Y_t)-\tf(X_t)$ by $O(\delta)$. \citet{saha2011improved} has a bound $O(\delta^2)$, using the special structure of $\tf$.
