...
the gradient estimates show different bias-variance tradeoffs and, correspondingly,
in previous works these algorithms were proven to achieve different convergence rates.
 In this paper we propose a novel framework that replaces the specific gradient estimation
 methods with an abstract oracle model capturing the controlled bias-variance tradeoff
 of existing gradient estimators. With the help of the new framework we unify previous works,
 reproducing  previous results in a clean and concise fashion, 
 while, perhaps more importantly, the framework also allows us to formally show that to achieve the optimal 
  ...
  or the proofs have to go beyond what has been used up to now in the literature.
\if0
