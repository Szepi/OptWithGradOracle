...
The main novelty of the model is that the information flow between the algorithm and the environment (holding $f$, or $f_{1:n}$) is mediated by the gradient estimation oracle. As we shall see, numerous existing approaches to online learning and optimization based on noisy pointwise information fit in this framework.

We will use two classes of oracles. In any case, the oracles are specified by
...
\todoc{They are NOT stated separately now!!!!!}
...
Examples of oracles for various $(c_1,c_2)$ are discussed in  \cref{sec:sbco}.
...
In this paper we will study the minimax regret in the online convex optimization setting, \todoc{BCO: I took it off as it was  confusing: What is ``bandit'' here?}
...
The minimax regret for type-II (and other similar) oracles is defined analogously (in what follows we only define these quantities for type-I oracles, as the extension \todoc{Is extension the right word?}
