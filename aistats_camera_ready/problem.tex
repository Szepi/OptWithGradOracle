...
Note that $R$ may depend on the dimension $d$, e.g., for $\mathcal{K} = \left[ -1,1 \right]^d$, $R = \sqrt{d}$ when using the Euclidean norm. To clarify the different $c_1$ used by type-I and II oracles, we will present the upper and lower bounds separately for the two oracles, although the type-I upper bound can actually be derived from type-II (and the type-II lower bound can be derived from type-I). \todox{I mentioned the different $c_1$  here.}
