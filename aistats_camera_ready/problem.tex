...
We call an oracle (type-I or II) has \emph{unbiased evaluation} if $\EE{Y}=x$ in the above definitions. Note that if the oracle has unbiased evaluation, an algorithm does not loose too much from suffering loss at $Y$ instead of the query point $x$ if the loss function is smooth, since in this case
