...
$D_f(x,y) \le \tfrac{L}{2} \|x-y\|^2$, for all $x,y \in \K$.
 ...
 The set of $L$-smooth functions is thus $\F_{L,0}(\K)$. Furthermore, let $\F_{L,\mu,\cR}(\K)$ denote the set of functions that are  $L$-smooth and $\mu$-strongly convex w.r.t.\  $\cR$, and let $\C^3$ denote the set of three times continuously differentiable functions.
% We let $\normal(0, I)$ denote the standard normal distribution in $d$-dimensions.
\todoa{This will be needed in the long version}
