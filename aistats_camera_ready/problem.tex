...
\vspace{-0.1cm}
\end{definition}
Thus, while a type-I oracle returns a biased, noisy gradient estimate for $f$, 
...
We call an oracle (type-I or II)  \emph{unbiased} if $\EE{Y}=x$ in the above definitions. Note that if the oracle is unbiased, an algorithm does not loose too much from suffering loss at $Y$ instead of the query point $x$ if the loss function is smooth, since in this case
$\EE{f(Y)}-f(x) \le \EE{\ip{\nabla f(x), Y-x}+\tfrac{L}{2}\norm{Y-x}} \le L\delta^2/2$.
Also, our definitions allow the same oracle $\gamma$ to respond to the same inputs $(x,\delta,f)$ with a differently constructed pair (e.g., to have memory),
...
In particular, a type-I oracle is also a type-II oracle. On the other hand, type-II oracles need to satisfy an alternative condition to become type-I oracles:
...
where the expectation is over the randomness of $G$. 
We immediately see that \todoc{This is not true! $G$ changes with $x$...!?!?}
\[
 \nabla \tilde{f}(x) = \EE{\nabla f(x)+ \left( G-\nabla f(x)\right)}
 =\EE{G}\,.
 \]
...
the performance of an algorithm is evaluated using the minimax \emph{regret} 
 in the stochastic convex optimization setting,
the minimax \emph{error}
 (sometimes, also called as the ``simple regret'') serves as the basis of comparing algorithms.
...
Consider now the case when the set $\K$ is bounded and for simplicity assume that $\sup_{y\in \K} \norm{y}=1$.
