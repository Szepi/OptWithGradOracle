...
Next we will see how this affects the achievable error for any algorithm. It is obvious that $f_v$ is minimized at $x^*_v = v$, with $f_v(v) = 2 \epsilon^2 \ln 2$.
...
We will consider the oracles $\gamma_v$ defined as \todoc{There was a sign mistake here, oh my!}
\begin{align}
 \gamma_v(x,\delta)
 	 = 
	 \underbrace{
	 f_v(x)
%	\epsilon \,\dfrac{1-e^{-\frac{x-v}{\epsilon}}}{1+e^{-\frac{x-v}{\epsilon}}} 
...
Now, observe that 
$\max_{\delta> 0} \{(\epsilon - C_1 \delta^p)_+\}^2 \delta^q = \max_{(\epsilon/C_1)^{1/p}>\delta> 0} (\epsilon - C_1 \delta^p)^2 \delta^q$ (since the maximum is obviously positive). 
