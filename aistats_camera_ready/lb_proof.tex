...
For $v\in \{\pm 1\}$ let
\begin{align*}
f_v(x) :=  \epsilon\left( x-v\right)+2\epsilon^2 \ln\left(1+e^{-\frac{x-v}{\epsilon}}  \right)\,, \qquad x\in \K\,.
...
We will consider the oracles $\gamma_v$ defined by \todoc{There was a sign mistake here, oh my!}
\begin{align}
 \gamma_v(x,\delta)
 	 = \overline{\gamma}_v(x,\delta)+ \xi_\delta, \label{eq:main:oracle-1d}
\end{align}
where $\xi_\delta \sim \normal(0,\frac{C_2}{\delta^q})$
and it remains to define $\overline{\gamma}_v$.
...
\overline{\gamma}_+(x,\delta) = 
	...
	\min(f_+'(x) + \min(\epsilon,C_1\delta^p), & \\
	\quad\quad f_-'(x) - \min(\epsilon,C_1\delta^p))\,, & \text{otherwise}\,,
	\end{cases}
\end{align*}
and
\begin{align*}
\overline{\gamma}_-(x,\delta) = 
	\begin{cases}
	f_-'(x) - \min(\epsilon,C_1\delta^p)\,, & \text{if } x>0\,; \\
	\max(f_-'(x) - \min(\epsilon,C_1\delta^p), & \\
	\quad\quad f_+'(x) + \min(\epsilon,C_1\delta^p))\,, & \text{otherwise}\,.
	\end{cases}
\end{align*}
...
Fixing $x$, $\delta$, $\epsilon$, 
we will write $f_v'$ in place of $f_v'(x)$ and $\overline{\gamma}_v$ for $\overline{\gamma}_v(x,\delta)$. 
We also let $c=\min(\epsilon,C_1\delta^p)$.
Thus, our goal is to show $|\overline{\gamma}_+-f_+'|\le c$.
When $\overline{\gamma}_+= f_+' + c$, the difference in question is clearly bounded by $c$. 
The remaining case has two subcases: When $\overline{\gamma}_+ \ge f_+'$, 
$|\overline{\gamma}_+-f_+'| = \overline{\gamma}_+-f_+' = \min(f_+'+c,f_-'-c)-f_+' \le f_+' + c - f_+'\le c$.
Finally, when $\overline{\gamma}_+>f_+'$ and $\overline{\gamma}_+=f_-'+c$,
$|\overline{\gamma}_+-f_+'| = f_+' - \gamma_+  = f_+'-f_-'+c \le c$, where the last inequality
follows since $f_+'\le f_-'$.
...
& \le 2(\epsilon - C_1\delta^p)_+\,, \numberthis \label{eq:gdiff-ub}
\end{align*}
where $a\wedge b = \min(a,b)$ and $(u)_+ = \max(u,0)$ is the positive part of $u$.
