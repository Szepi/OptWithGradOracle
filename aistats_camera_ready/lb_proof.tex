...
Now, observe that 
$\sup_{\delta> 0} \{(\epsilon - C_1 \delta^p)^+\}^2 \delta^q = \sup_{(\epsilon/C_1)^{1/p} \ge \delta> 0} (\epsilon - C_1 \delta^p)^2 \delta^q$,
we obtain
\begin{align}
...
Now, when $q=2$ and $p=1$, the lower bound in \eqref{eq:lb-pq} simplifies to \todoc{I edited the lower bound on $n$. It was 
$\frac{2C_2}{9 C_1^2}$.}
\[
...
\[
\Delta_n^{(1)*} \ge  \frac{9}{10}\left(\frac{1}{25}\right)^{1/3}C_1^{1/3}C_2^{1/3} n^{-1/3}\,.
\]
\todoa[inline]{This is where I stopped. The results are not used below and not copied to the theorem before somebody checks them. Some multiple of 2 difference to the previous version...}
\todoc[inline]{I also stopped here for now. The results can be copied once my single change above is verified.}
...
\begin{align*}
f_v(x) = \sum_{i=1}^d f^{(i)}_{v_i}(x_i), \qquad x\in \cK\,.
\end{align*}
We also defined a
...
oracle $\gamma_{v^*_{-i}}^{(i)}$, \todoc{this is not formally defined, but I think this should be clear enough.}
...
Here, we assume that all oracles use independent random noise. \todoc{Actually, this does not matter it seems. This is a bit weird.}
...
Let $\Delta_n^{(d)*}$ denote the minimax error $\Delta_n^*\left(\F_d, C_1\delta^p,\frac{C_2}{\delta^q}\right)$ for the $d$-dimensional family of functions $\F_d$. Let $\F_1 = \{ f^{(1)}_{+1},  f^{(1)}_{+1} \}$.
...
\todoc[inline]{I fixed the $d$ dim. proof up to this point.}
...
\begin{align*}
  f_v(x) := \dfrac{1}{2} x^2 - v\epsilon x \,\, x \in \cK\,.
\end{align*}
By assumption,  $\{f_+, f_-\}\subset \F$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\begin{align}
 ...
 \le & \dfrac{2(\epsilon-C_1\delta_t^p)_+^2\delta_t^q}{C_2},\label{eq:dkgausssc}
\end{align}
where the inequality \eqref{eq:dkgausssc} follows from \eqref{eq:gdiff-ub-sc}.
Thus, we obtain
\begin{align}
\dkl{P_{+}}{P_{-}} \le\sup_{\delta>0} \dfrac{2n(\epsilon-C_1\delta^p)_+^2 \delta^q}{C_2}.
\end{align}
Substituting the above bound into \eqref{eq:pinskersc}, we obtain 
\begin{align}
 \Delta_n^{*}
  \ge & \dfrac{\epsilon^2}{2} \left(1 - \sqrt{
    n}  \sup_{\delta>0}\dfrac{(\epsilon-C_1\delta^p)_+\delta^{q/2}}{C_2}
...
\begin{align}
\delta^*=\left(\frac{ \epsilon q}{C_1(2p+q)}\right)^{1/p}\,.
...
\[
\Delta_n^{(1)*} \ge \dfrac{\epsilon^2}{2} \left(1 - \sqrt{n}  K_1 \epsilon^{\frac{p+\tfrac{q}{2}}{p}}\right), \text{ where } K_1 = \dfrac{p}{\sqrt{C_2}(p+\tfrac{q}{2})} \left(\dfrac{q}{2C_1(p+\tfrac{q}{2})}\right)^{\frac{q}{2p}}.
\]


Plugging in $\epsilon = \left(\dfrac{4p}{(6p+q)\sqrt{n} K_1} \right)^{\frac{2p}{2p+q}}$, we obtain
\begin{align}
\Delta_n^{(1)*} \ge 2^{\frac{2p-q}{2p+q}}\left( \frac{2p+q}{6p+q} \right)^{\frac{6p+q}{2p+q}}\left( \frac{2p+q}{q} \right)^{\frac{2q}{2p+q}} C_1^{\frac{2q}{2p+q}}C_2^{\frac{2p}{2p+q}} n^{-\frac{2p}{2p+q}}.
\label{eq:lb-pq-sc}
\end{align}
...
On the other hand, for $p=q=2$, we obtain
\[
\Delta_n^{(1)*} \ge 27(\frac{2}{7^7})^{\frac{1}{3}} C_1^{2/3}C_2^{2/3} n^{-2/3} .
\]
...
\paragraph{Generalization to $d$ dimensions:}
The proof in $d$ dimensions for strongly convex functions is the same as that for the case of convex functions and we obtain the following rate for
general $p, q >0$:
\begin{align}
