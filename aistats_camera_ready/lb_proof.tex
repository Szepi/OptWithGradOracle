   ...
   \begin{axis}[width=9cm,height=5cm,
          %  grid = major,         
           ...
           \addplot[domain=-2.75:5, green!35!black, thick,smooth] 
              {x-1 + 2*ln(1+exp(-(x-1)))} node [pos=0.9,pin={135:$f_+$}] {} node [pos=0.1,pin={85:{\makecell{\scriptsize decreasing\\\scriptsize when $x\!<\!0$}}}] {}; 
            \addplot[domain=-5:2.75, red!35!black,thick,smooth] 
              {x+1 + 2*ln(1+exp(-(x+1)))} node [pos=0.1,pin={45:$f_-$}] {} node [pos=0.615,pin={5:\makecell{\scriptsize$\min\limits_{x<0} f_+(x)$}}] {}; 
   \end{axis}
   \end{tikzpicture}}
	\caption{Plot of $f_+$ and $f_-$ with $\epsilon=1$}
\label{fig:lbfvdiff}
	  \end{subfigure}
	&
	  \begin{subfigure}[b]{0.5\textwidth}
 \centering
   ...
   \begin{axis}[width=9cm,height=5cm,
          %  grid = major,         
           ...
           \addplot[domain=-1.5:5, green!35!black, thick,smooth] 
              {1-exp(-(x-1))/(1+exp(-(x-1)))} node [pos=0.5,pin={315:$f'_+$}] {}; 
            \addplot[domain=-5:1.5, red!35!black,thick,smooth] 
              {1-exp(-(x+1))/(1+exp(-(x+1)))} node [pos=0.4,pin={135:$f'_-$}] {}; 
							%\node at (axis cs:1,0.5) {P3};
   \end{axis}
   \end{tikzpicture}}
	\caption{Plot of $f'_+$ and $f'_-$ with $\epsilon=1$}
	\label{fig:fprime}
\end{subfigure}
	\end{tabular}
\end{figure}
...
Let $h(v) = -v + 2\epsilon\ln\dfrac{1+e^{\frac{v}{\epsilon}}}{2}$. In the following, we establish that  $h(v)$ is an even function, i.e., $h(v) = h(-v)$.
\begin{align*}
