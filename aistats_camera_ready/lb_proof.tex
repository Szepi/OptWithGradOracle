...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
In this section we present the proof of Theorem~\ref{thm:lb-convex}. 
Note that we will only prove lower bounds with the type-I oracle. According to Theorem~\ref{thm:typered}, lower bounds for type-II can be directly attained by replacing $C_1$ of type-I with $C_1/\sqrt{d}$, given $[+1,-1]^d\subset \cK $.
...
These functions, with the choice $\epsilon=0.1$, are shown in Figure~\ref{fig:lbfvdiff}. 
...
The derivatives of $f_+$ and $f_-$ are shown in Figure~\ref{fig:lbfvdiff}; we define the ''bias'' in $\og_v$ to move the gradients closer to each other:
...
shifted down version of $f_-'$, cf. Figure~\ref{fig:fprime}.
...
\end{center}
\caption{The construction of algorithm $\A_i^*$ used in the proof of Lemma~\ref{lemma:sep}.}
...
Now, algorithm $\A_i^*$ is constructed as illustrated on Figure~\ref{fig:sepalgconstruction}.
...
\begin{proof}(Theorem~\ref{thm:lb-convex})
...
\subsection{Proof of Theorem~\ref{thm:lb-convex} for strongly convex and smooth function class $\F_{L,1}(\K)$.}
