...
\begin{equation}
\label{eq:oracle}
G(x,\delta) = \overline{\gamma}(x) + \xi
\end{equation}
with some map $\og: \cK \to \R$,
...
Next we define the two target functions and their associated oracles. With a slight abuse of notation, we will use interchangeably the subscripts $+$ ($-$) and $+1$ ($-1$) for any quantities corresponding to these two environments, e.g., $f_+$ and $f_{+1}$ ($f_-$ and $f_{-1}$).
For $v \in \{\pm 1\}$, let
\begin{align*}
...
The derivatives of $f_+$ and $f_-$ are shown in \cref{fig:lbfvdiff}; we define the bias of $\og_v$ to make the gradients more similar:
...
shifted down version of $f_-'$, cf. \cref{fig:fprime}.
 By moving the curves towards each other, algorithms which rely on the oracles
...
\overline{\gamma}_-(x,\delta) = 
	...
	\end{cases}
\end{align*}
We claim that this oracle is indeed a $(c_1,c_2)$ type-I oracle, with $c_1(\delta)=C_1\delta^p$ and $c_2(\delta)=\frac{C_2}{\delta^q}$. The variance condition is trivial.
...
Given the above definitions, by Yao's principle, the minimax error \eqref{eq:minimaxerrdef} is lower bounded by
...
$\Omega = \R^n\times \{-1,1\}$,  its associated Borel sigma algebra $\B$,
and $P_{\A,\gamma}$ defined by $P_{\A,\gamma} := p_{\A,\gamma} d(\lambda \times m)$, 
where
...
&=  \frac{1}{2} \bigg( p_{\N}\big(g_n - \overline{\gamma}(x_n^{\A}(g_{1:n-1})),c_2(\delta_n^{\A}(g_{1:n-1}))\big) \cdot
%  &\times p_{\normal(0,\sigma^2)}(g_{n-1} - \gamma(\A_n(g_{1:n-2}))) \times \ldots \\
									 \ldots \cdot  p_{\N}\big(g_1 - \overline{\gamma}(x_1^{\A}),c_2(\delta_1^{\A})\big) \bigg),
\end{align*}
where $v\in\{-1,1\}$ and $p_{\N}(\mu,\sigma^2)$ is the density function of a $\normal(\mu,\sigma^2)$ random variable.
...
Combining \eqref{eq:fv-lb} and \eqref{eq:avg-bd}, we obtain
\begin{align}
...
\end{align}
where 
the equality in \eqref{eq:Pplus} uses the definitions of $\P_+$ and $\P_-$, $\tvnorm{\cdot}$ denotes the total variation distance and  
 \eqref{eq:lecam} follows from its definition, while \eqref{eq:pinsker} follows from Pinsker's inequality. %\todoc{You lost a factor of two here!}
...
\[
\Delta_n^{*} \ge \dfrac{ 2(C_1^2C_2)^{1/4}}{3\sqrt{3}} n^{-1/4}, \text{ for } n \ge \frac{2C_2}{9 C_1^2}~.
\]
