...
By continuity, the above claim can be extended to cover the case of $q=0$. For the special case of $p=0$ and $C_1>0$, which implies a constant bias, it is possible to derive to an $\Omega(1)$ lower bound by tweaking the proof. On the other hand, the case of $p=0$ and $C_1=0$ leads to an $\Omega(d/\sqrt{n})$ lower bound.
