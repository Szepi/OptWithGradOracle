...
In this section we present the proofs corresponding to the oracles introduced in Section~\ref{sec:sbco}.
...
\begin{proof}
Given that $\EE{\norm{V}_*^2}$ and $\EE{\xi^2}$ are bounded, the variance of $G$ remains the same as stated in Proposition~\ref{prop:grad-onepoint}.
...
\end{align*}
where the second equality follows from Lemma~\ref{lem:gradientCalculus}, by replacing the gradient of $\hat{f}(u) =\dfrac{1}{\delta} f(x+\delta u) $ with $\nabla f(x+\delta u)$.
