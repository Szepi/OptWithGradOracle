...
If $f$ is smooth, further assuming $W$ is symmetric w.r.t. origin, $\gamma$ is a type-II, and also a type-IIb oracle with $c_1(\delta) = C_1\delta^2$, $c_2(\delta) = C_2/\delta^2$. Hence, in this case $\gamma$ is also a type-I oracle with the same $c_1$ and $c_2$.
\end{proposition}
\begin{proof}
See Section \ref{sec:appendix-grad}.
\end{proof}
