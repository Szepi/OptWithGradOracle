...
\node [mybox] (box2) {%
    \begin{minipage}[t!]{.91\textwidth}
     When $p=q=2$, recover the upper bound $O(T^{-1/3})$ for smooth functions, and $O(T^{-1/2})$ for smooth \& strongly convex functions\\
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\headerbox{Conclusion and Future Work}{name=finalbox,column=2,below=upperbound,span=2, headerColorOne=teagreen, borderColor=teagreen}{
\begin{itemize}
\item \textbf{Partition:} gradient estimate + gradient-based algorithm. Abstract gradient estimates away from the algorithm. Directly feed the algorithm with biased, noisy gradient estimate.
\end{itemize}
%$\Longrightarrow$ Abstract gradient estimates away from the algorithm. Directly feed the algorithm with biased, noisy gradient estimate.
\begin{itemize}
\item \textbf{Necessary Sub-optimal?} If a proof for a gradient method uses only the stated bias-variance tradeoff, then the stated lower bound applies.
\item \textbf{Better gradient estimate exists?} 
\end{itemize}
