...
\subsection{Proof of \cref{thm:lb-convex} for the class of smooth convex functions $\F_{L,0}(\K)$}
\label{sec:appendix-lbconvex}
We will use a novel technique that will allow us to  reduce the $d$-dimensional case to the one-dimensional case (see later).
...
Thus, $\gamma_v$ is indeed an oracle with the required properties.
 ...
 \begin{figure}
   \centering
	\begin{tabular}{cc}
	  \subfigure[Plot of $f_+$ and $f_-$ with $\epsilon=0.1$]{
\label{fig:lbfvdiff}
 \centering
   ...
   \end{tikzpicture}}
	...
	&
\subfigure[Plot of $f'_+$ and $f'_-$ with $\epsilon=0.1$. 
...
\end{align*}
%Above, the equality in \eqref{eq:lbfvdiff} can be inferred as follows: Suppose $v=+1$. As illustrated in \cref{fig:lbfvdiff}, $\min_{x:xv<0} f_v(x)$ is achieved at $x=0$ due to monotonicity of $f_v$. A similar argument works for $v=-1$ as well. 
%This can also be seen from the plot of $f_+$ to the left of $+1$ and of $f_-$ to the right of $-1$ in \cref{fig:lbfvdiff}. 
...
%\end{align}
%The above conditions on $n$ can be inferred from (i) the requirement on  $\epsilon$ in \eqref{eq:deltastar}, (ii) assumption $\delta \le 1$ in \cref{def:oracle1},  and (iii) the optimal choice of $\epsilon$ above.
...
\begin{proof}(\cref{thm:lb-convex})
...
\subsection{Proof of \cref{thm:lb-convex} for strongly convex and smooth function class $\F_{L,1}(\K)$.}
...
%As in the proof of the lower bound for $\F_{L,0}(\K)$, the first condition on $n$ above is due to the assumption that $\delta \le 1$ in \cref{def:oracle1}, while the second condition is due to the constraint on $\nu$ in \eqref{eq:deltastar-sc}.
