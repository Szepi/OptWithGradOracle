...
$\gamma_v(x) = \sum_{i=1}^d \gamma_{v_i}^{(i)}$, where $\gamma_{v_i}^{(i)} = \epsilon(x_i-v_i) + v_i\, C_1 \delta^p$. 
It is evident that the $\gamma$ is a $(c_1,c_2)$ Type-I oracle.
...
while the inequality \eqref{eq:dlb} follows from the fact that each infimum term in \eqref{eq:splitA} is solving a $1$-dimensional problem and hence the one-dimensional minimax error $\Delta_n^{(1)*}$ applies in each dimension. 
