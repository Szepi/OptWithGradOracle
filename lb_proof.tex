...
\Delta_n^{(1)*}  \ge & \inf_{\A} \dfrac{\epsilon}{2}\,  \P(\hat X_n V < 0), \label{eq:strong-convex-bd}\\
  ...
  \ge &\inf_{\A} \dfrac{\epsilon }{2}  \,\left( 1 - \left(2\ln 2\dkl{P_{+}}{P_{-}}\right)^{\frac{1}{2}}\right), \label{eq:pinsker}
\end{align}
where 
...
Let $P_{+}^t(\cdot\mid g_1,\ldots,g_{t-1})$ denote the distribution of $G_t$ conditional on $V=+1$ and $G_1=g_1,\ldots,G_{t-1}=g_{t-1}$. Define  $P_{-}^t(\cdot\mid g_1,\ldots,g_{t-1})$ in a similar fashion.
Then, by the chain rule for KL-divergences, we have
\begin{align}
\label{eq:dklchain}
&\dkl{P_{+}}{P_{-}}= \sum_{t=1}^n \int_{\R^{t-1}} \dkl{P_{+}^t(\cdot\mid g_{1:t-1})}{P_{-}^t(\cdot\mid g_{1:t-1})} d P_{+}^{t-1}( g_{1:t-1}).
\end{align}
...
\begin{align}
 \Delta_n^{(1)*}
  \ge & \dfrac{\epsilon}{2} \left(1 - 2 \sqrt{
    n}  \dfrac{\max_{\delta>0} (\epsilon-C_1\delta^p)_+\delta^{q/2}}{\sqrt{C_2}}
...
Observe that $\max_{\delta> 0} (\epsilon - C_1 \delta^p)_+ \delta^{q/2} = \max_{\delta> 0} (\epsilon - C_1 \delta^p) \delta^{q/2}$. 
...
Plugging in $\epsilon = \left(\dfrac{1}{\sqrt{n} K_1} \right)^{\frac{p}{p+\frac{q}{2}}}$, we obtain
\begin{align}
