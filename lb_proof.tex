...
We will consider the oracles $\gamma_v$ defined as \todoc{There was a sign mistake; we need $\dots - v \min(\dots)$.}
\begin{align}
...
Observe that $\max_\delta (\epsilon - C_1 \delta^p)_+ \delta = \max_\delta (\epsilon - C_1 \delta^p) \delta$. 
...
\todoc{This calls for a little explanation because of the positive part.
The expression without the positive part has the minimizer below. This minimizer is always small enough, so that 
we are not in the $0$ arm of the positive part function. Further, the minimizer can never be in the zero arm
of this function. Therefore, the minimizer is indeed $\delta^*$.}
\[
