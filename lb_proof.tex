...
by $\gamma_v(x) = \epsilon(x-v) + v\, C_1 \delta^p$ 
 (as with $f_v$, we will also use $\gamma_{+}$ ($\gamma_-$) 
...
To summarize, \todoc{Maybe this could have been stated above formally once and referred to here only, saving space.}
...
\todoc{Hmm, do the oracles have the same bias/variance properties?}
