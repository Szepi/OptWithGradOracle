...
Now, for Algorithm~2, using the parameter choice $\delta=T^{-\frac{3}{16}}$ in \citet{DekelEK15}, we can choose $\epsilon=T^{-\frac{5}{16}}$, leading to the lower bound $\EE{R} = \Omega(T^{-5/16})$, contradicting the upper bound $O(T^{-3/8})$ of \citet{DekelEK15}.
