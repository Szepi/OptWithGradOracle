...
\subsection{Results for stochastic BCO}
...
For stochastic BCO with uncontrolled noise, an algorithm based on random walks (and not using gradient estimates) is proposed by \cite{liang2014zeroth}.
...
which decreases faster in $n$ than the bound available in the above result, while showing a much worse dependence on the dimension.\todoc{We should show the dimension dependence in the result to make this point. This should be shown for the same setting as the one considered by \cite{liang2014zeroth}.}
