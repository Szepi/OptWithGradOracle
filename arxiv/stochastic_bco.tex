...
\paragraph{Connecting our results to stochastic BCO:}
...
$(\delta,1)$ type-I oracle\todoc{Can we show that no better oracles exist? Can we show that $p=q=2$ is the best the oracle defined above can get? Then this statement could be much strengthened.}
...
\todoc[inline]{
The following statement should go: {\em For the noisy optimization setting, the $O(n^{-1/3})$ rate is known to be optimal for the smooth, convex case
by a result due to \citet{Chen88:LB-AoS}.}
But Im not sure which is the correct reference for stochastic BCO lower bound:
}
...
We next state a result with matching upper bound and this perhaps appeared first in an asymptotic form in the paper by \citet{PoTsy90}.
\begin{theorem}
