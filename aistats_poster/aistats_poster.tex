...
\headerbox{Upper Bound}{name=upper_bound,column=1, below=gradient}{
...
Tight for $\F_{L,0}$. Weak for $\F_{L,1}$.\\[1ex]

\textbf{Proof idea:}\\
Define $\F_{L,0}$ to be comprised of $f_v$ for $v \in \{-1,+1\}$.
\begin{align*}
f_v(x) :=\epsilon\left( x-v\right)+2\epsilon^2 \ln\left(1+e^{-\frac{x-v}{\epsilon}}  \right)\,,
\end{align*}
which is the convex relaxation of $\epsilon \left| x-v \right|$.
...
\headerbox{Conclusions}{name=conclusions,column=1, below=upper_bound}{
