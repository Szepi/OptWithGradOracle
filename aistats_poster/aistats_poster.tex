...
We know the rate $\Delta_n =\tilde{O}(\sqrt{1/n}) $, but it is non-constructive. {\tiny \cite{BubeckDKP15}}

\textbf{\textit{\color{blue}Controlled Noise}}: $\Delta_n \leq C\sqrt{d^2/n}$. {\tiny \cite{duchi2015optimal}}

\textbf{\textit{\color{blue}Uncontrolled Noise}}:\\
Ellipsoid method and relatives:
\begin{itemize}\compresslist
\item  $\color{blue}\sqrt{d^{33}/n}$.  {\tiny \cite{AgFoHsuKaRa13:SIAM}}
\item  $\color{blue}\sqrt{d^{14}/n}$.  {\tiny \cite{liang2014zeroth}}
\end{itemize}

``Gradient" methods:
\begin{itemize}\compresslist
\item \textbf{Convex}: $\color{blue}\left( d^2/n \right)^{1/4}$.
			{\tiny \cite{flaxman2005online}}
\item \textbf{Smooth}: $\color{blue}\left( d^2/n \right)^{1/3}$.
			{\tiny \cite{saha2011improved}}
\item Smooth + SOC: $\color{red}\sqrt{d^2/n}$.
			{\tiny \cite{HaLe14:SOC}}
\end{itemize}

Lower Bound: $\color{red}\sqrt{d^2/n}$. {\tiny \cite{shamir2012complexity}}
\begin{center}
\tikz[baseline]{
...
\headerbox{Oracle Model}{name=oracles,column=1,span = 2}{
\begin{center}
...
\end{center}
	...
	\end{itemize}
	\vspace{-1ex}
	\textbf{Does it matter which of these we select?\\ Not really:} when $f\in \mathcal{C}^3$\\
...
\toprule
  &{$\F_{L,\mu}$} & {$\F_{L,0}$}\\
\midrule
{\small\textit{Uncontrolled noise}}& \multirow{2}{*}{$O(n^{-1/2})$} &\multirow{2}{*}{$O(n^{-1/3})$}  \\
 \footnotesize$(p=2, q=2)$  &  &   \\
{\small\textit{Controlled noise}}& \multirow{2}{*}{$O(1/n)$} &\multirow{2}{*}{$O(n^{-1/2})$}  \\
 \footnotesize$(p=2, q=0)$  &  &   \\
\bottomrule
\end{tabular}
\end{center}
...
The lower bound for $\F_{L,0}$ is tight, for $\F_{L,1}$ it is loose.\\[0.5ex]

\textbf{\textsc{Idea:}} \\
Construct two loss functions, whose noisy gradient estimates are so close that they can NOT be distinguished.\\

Choose the smooth approximation of $\epsilon \left| x-v \right|$ for $v \in \{-1,+1\}$:
\begin{align*}
...
\begin{multicols}{2}
Notice that  $f_+$, $f_- \in \F_{L,0}$. 
\begin{center}
\begin{tikzpicture}
   \begin{axis}[width=7cm,height=3cm,
          %  grid = major,         
            axis y line=middle,
            axis x line=bottom,
						xlabel={$x$},
           ...
           \addplot[domain=-2.75:5, green!35!black, thick,smooth] 
              {0.1*(x-1) + 2*0.01*ln(1+exp(-10*(x-1)))}  ; 
...
\begin{tikzpicture}
   \begin{axis}[width=7cm,height=3.5cm,
          %  grid = major,         
           ...
           \addplot[domain=-5:5, green!35!black, thick,smooth] 
              {0.1*(1-exp(-10*(x-1)))/(1+exp(-10*(x-1)))} node [pos=0.59,pin={350:$f'_+$}] {}; 
   ...
   \end{tikzpicture}
\end{center}
The noisy oracle (dashed line) shifts the gradient (smooth line) with a bias.\\
Denote $b=\min\left(\epsilon,C_1\delta^p\right)$. Define
%\begin{align*}
