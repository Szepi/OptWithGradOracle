...
that is three times continuously differentiable with bounded third derivative and also satisfies  
$\E \left|f(x)\right|^{2+\zeta} < \infty$, for any $x\in \D$.
...
G =  V \left(\dfrac{f(x+\delta U) + \xi^+ - (f(x-\delta U) + \xi^-)}{2\delta}\right),
 \label{eq:twosp}
\end{align}
where $\xi^+, \xi^-$ are  i.i.d. noise terms that are zero-mean and satisfy 
$\E \left|\xi^{\pm}\right|^{2+\zeta} < \infty$, for some $\zeta>0$. 
Further, $V, U \in \R^d$ are jointly distributed random vectors 
that satisfy $\E[V U\tr] = I$, $U_i, i=1,\ldots,d$ are i.i.d., $|U_i|$ and $\E|V_i|$ have finite upper bounds. 
Then, we have that $\gamma$ is a type-I oracle with $c_1(\delta) = C_1 \delta^2$ and $c_2(\delta) = C_2 d/\delta^2$.
\end{proposition}
\begin{proof}
