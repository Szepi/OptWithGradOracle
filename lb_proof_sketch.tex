...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
We first prove the theorem in an one-dimensional setting and generalize it later.
...
\begin{align}
  f_v(x) - f_v(x^*_v)
  = &  \dfrac{\epsilon}{2} (x - v)^2 \ge  \dfrac{\epsilon}{2}  \indic{x v  < 0}. \label{eq:fv-lb-main}
\end{align}
...
Using \eqref{eq:fv-lb-main}, the minimax error \eqref{eq:minimaxerrdef} can be lower bounded as follows
...
\begin{align}
\dkl{P_{+}}{P_{-}} \le \dfrac{2n(\epsilon-C_1\delta^p)_+^2 \delta^q}{C_2}.\label{eq:dkbd-main}
\end{align}
Note that the above bound holds uniformly over all algorithms $\A$. 
Substituting the above bound into \eqref{eq:pinsker-main}, we obtain 
\begin{align}
 \Delta_n^{(1)*}
...
where $\xi \sim \normal(0,\frac{C_2}{\delta^q})$.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
The KL-divergence bound (corresponding to \eqref{eq:dkbd-main}) turns out to be 
\begin{align}
\dkl{P_{+}}{P_{-}} \le \dfrac{2n(\nu-C_1\delta^p)_+^2 \delta^q}{C_2},\label{eq:dkbd-sc-main}
\end{align}
